...
export default function App() {
  ...
  const handleImported = async (data: NormalizedFamilyData, preview: ImportPreviewResult) => {
    ...
    const importBatch = createImportBatchFromPreview({ mode: 'simple_csv', preview, sourceLabel: 'かんたんCSV', fileNames: data.importBatch.source_name ? [data.importBatch.source_name] : [] });
  ...
  const applyStandardCsvSet = async () => {
    ...
    const importBatch = createImportBatchFromPreview({ mode: 'standard_csv_set', preview: standardPreview.preview, sourceLabel: '標準CSVセット', fileNames: ['manifest.json', ...(standardPreview.preview.files?.filter((file) => file.present).map((file) => file.fileName) ?? [])] });
