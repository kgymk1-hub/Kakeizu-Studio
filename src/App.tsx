...
import type { Citation, Event, ExportSetting, ImportBatch, ParentChildRelation, Person, PrivacySetting, Project, SelectableTarget, Source, Union, ValidationIssue, ViewSetting } from './models';
...
function ImportReportPanel({ report }: { report?: ImportReport }) {
  if (!report) return <section className="panel import-report"><h2>インポート結果レポート</h2><p>取込実行後に結果が表示されます。</p></section>;
  const unresolved = report.unresolvedReferenceSummary;
  return <section className="panel import-report"><h2>インポート結果レポート</h2><p><strong>取込結果:</strong> {importReportStatusLabel(report.status)} / <strong>取込元:</strong> {importModeLabel(report.mode)} / <strong>取込方式:</strong> {importBatchPolicyLabel({ id: '', imported_at: '', import_type: 'csv_simple', imported_count: 0, warning_count: 0, error_count: 0, import_policy: report.importPolicy })}</p><p><strong>仮人物方針:</strong> {report.placeholderPersonPolicy ?? '-'} / <strong>実行日時:</strong> {new Date(report.createdAt).toLocaleString()}</p>{report.fileNames.length > 0 && <p><strong>ファイル:</strong> {report.fileNames.join(', ')}</p>}<h3>取込件数</h3><div className="preview-metrics"><span>Person {report.importedCounts.persons}</span><span>Union {report.importedCounts.unions}</span><span>Relation {report.importedCounts.relations}</span><span>Source {report.importedCounts.sources}</span><span>Citation {report.importedCounts.citations}</span><span>Event {report.importedCounts.events}</span></div><h3>issue概要</h3><div className="preview-metrics"><span>warning {report.issueSummary.warningIssues}</span><span>error {report.issueSummary.errorIssues}</span><span>total {report.issueSummary.totalIssues}</span></div>{report.matchSummary && <><h3>external_id照合</h3><div className="preview-metrics"><span>新規候補 {report.matchSummary.newItems}</span><span>既存一致候補 {report.matchSummary.matchedExisting}</span><span>CSV内重複 {report.matchSummary.duplicateInImport}</span><span>external_idなし {report.matchSummary.missingExternalId}</span></div></>}{report.policyPlan && <><h3>取込方式ごとの予定処理</h3><div className="preview-metrics"><span>作成 {report.policyPlan.create}</span><span>更新 {report.policyPlan.update}</span><span>スキップ {report.policyPlan.skip}</span><span>別ID追加 {report.policyPlan.addAsNew}</span><span>全置換 {report.policyPlan.replace}</span><span>保留候補 {report.policyPlan.blocked}</span></div></>}{unresolved && <><h3>参照先不明</h3><div className="preview-metrics"><span>合計 {unresolved.total}</span><span>人物参照 {unresolved.personReferences}</span><span>Source {unresolved.sourceReferences}</span><span>Event {unresolved.eventReferences}</span><span>Union {unresolved.unionReferences}</span><span>Relation {unresolved.relationReferences}</span><span>仮人物候補 {report.placeholderPersonCandidateCount}</span><span>保留 {unresolved.pendingReview + unresolved.blockedByPolicy}</span></div></>}<h3>次に確認すること</h3><ul className="compact-list">{report.nextActions.map((action)=><li key={action}>{action}</li>)}</ul><h3>主なissue</h3>{report.issuePreview.length === 0 ? <p>問題はありません。</p> : <ul className="issue-list">{report.issuePreview.map((i, idx)=><li key={idx} className={i.severity}>{i.severity} [{i.code}] {i.fileName && `${i.fileName} / `}{i.rowNumber && `${i.rowNumber}行目 / `}{i.field && `${i.field}: `}{i.targetId && `${i.targetId} / `}{i.message}</li>)}</ul>}<h3>主な参照先不明</h3>{report.unresolvedReferencePreview.length === 0 ? <p>参照先不明はありません。</p> : <ul className="compact-list">{report.unresolvedReferencePreview.map((ref, idx)=><li key={idx}>{ref.fileName && `${ref.fileName} `}{ref.rowNumber && `${ref.rowNumber}行目 `}{ref.field}: {ref.referenceId} / 参照元: {ref.sourceDisplayName ?? ref.sourceImportId ?? ref.sourceEntityType}</li>)}</ul>}<h3>仮人物候補</h3>{report.placeholderPersonCandidatePreview.length === 0 ? <p>仮人物候補はありません。</p> : <ul className="compact-list">{report.placeholderPersonCandidatePreview.map((candidate)=><li key={candidate.referenceId}>{candidate.displayName} ({candidate.referenceId}) / 参照 {candidate.references.length}件</li>)}</ul>}<p className="help-text">詳細レポートはDB永続保存せず、直近取込結果として画面上に表示します。</p></section>;
}
...
export default function App() {
  ...
  const applyStandardCsvSet = async () => {
    ...
    if (!standardPreview.preview.canImport) { setStatus(standardPreview.preview.summary.importPolicyStatus === 'preview_only' ? 'この取込方式は現在プレビューのみ対応です。実行はできません。' : 'エラーがあるため標準CSVセットを反映できません。'); return; }
