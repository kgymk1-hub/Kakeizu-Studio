...
import { CsvImport, ImportPolicySelector, sampleCsv } from './components/CsvImport/CsvImport';
...
import { buildPolicyPlan, canImportWithPolicy, getImportPolicyOption, getImportPolicyStatus, type ImportPolicy } from './services/importPreviewService';
...
export default function App() {
  ...
  const handleStandardImportPolicyChange = (nextPolicy: ImportPolicy) => {
    setStandardImportPolicy(nextPolicy);
    setStandardPreview((current) => current ? {
      ...current,
      preview: {
        ...current.preview,
        summary: { ...current.preview.summary, importPolicy: nextPolicy, importPolicyStatus: getImportPolicyStatus(nextPolicy), policyPlan: buildPolicyPlan(current.preview.matches, nextPolicy) },
        canImport: canImportWithPolicy(current.preview.summary.errorIssues, nextPolicy),
      },
    } : current);
  };
  ...
  const loadStandardCsvSetZipPreview = async (file: File) => {
    try {
      const preview = await parseStandardCsvSetZip(file, { importPolicy: standardImportPolicy, existingData: { persons, unions, parentChildRelations: relations, sources, citations, events } });
  ...
  const loadStandardCsvSetFilesPreview = async (files: FileList) => {
    try {
      const preview = await parseStandardCsvSetFileList(files, { importPolicy: standardImportPolicy, existingData: { persons, unions, parentChildRelations: relations, sources, citations, events } });
