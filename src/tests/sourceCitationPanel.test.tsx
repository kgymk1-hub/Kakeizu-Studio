...
describe('SourceCitationPanel', () => {
  ...
  it('SourceクリックでCitationをSource絞り込みする', () => { const { container, root } = renderPanel(); const button = Array.from(container.querySelectorAll('button')).find((item) => item.textContent?.includes('資料タイトル')) as HTMLButtonElement; act(() => { button.click(); }); expect(container.textContent).toContain('資料の絞り込みを解除'); act(() => root.unmount()); });
});
