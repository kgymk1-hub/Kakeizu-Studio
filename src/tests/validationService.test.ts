...
const source = (id = 's1', extra: Partial<Source> = {}): Source => ({ id, source_type: 'book', title: id, created_at: now, updated_at: now, ...extra });
...
describe('validateFamilyData', () => {
  ...
  it('Event.place_id / Source.place_idの参照切れをwarningで検出する', () => { const input = base(); input.events = [event('e1', { place_id: 'missing-place' })]; input.sources = [source('s1', { place_id: 'missing-place' })]; const issues = validateFamilyData(input).filter((i) => i.category === 'broken_reference' && i.related_ids?.includes('missing-place')); expect(issues).toEqual(expect.arrayContaining([expect.objectContaining({ severity: 'warning', target_type: 'event', target_id: 'e1' }), expect.objectContaining({ severity: 'warning', target_type: 'source', target_id: 's1' })])); expect(validateFamilyData({ ...input, places: [{ id:'missing-place', name:'東京', created_at:now, updated_at:now }] }).filter((i) => i.related_ids?.includes('missing-place'))).toEqual([]); });

  it('Name / Place targetのCitationは存在確認でき、参照切れを検出する', () => { const input = base(); input.citations = [citation('c1', { target_type: 'name', target_id: 'n1' }), citation('c2', { target_type: 'place', target_id: 'pl1' })]; expect(validateFamilyData(input).filter((i) => i.category === 'broken_reference')).toHaveLength(2); expect(validateFamilyData({ ...input, names: [{ id:'n1', person_id:'p1', name_type:'alias', name_text:'別名', created_at:now, updated_at:now }], places: [{ id:'pl1', name:'東京', created_at:now, updated_at:now }] }).filter((i) => i.category === 'broken_reference')).toEqual([]); });
