...
export const STANDARD_CSV_SET_FILES = ['persons.csv', 'unions.csv', 'parent_child_relations.csv', 'sources.csv', 'citations.csv'] as const;
const BOM = '\uFEFF';
...
export function parseStandardCsvSetFiles(files: Record<string,string>): StandardCsvSetPreview {
  ...
  for (const f of STANDARD_CSV_SET_FILES) if (!files[f]) issues.push({ severity:'error', code:'missing_csv', message:`${f} がありません。` });
