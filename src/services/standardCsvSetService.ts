...
export const STANDARD_CSV_COLUMNS = {
  ...
  unions: ['id','external_id','partner1_id','partner2_id','union_type','marriage_date_text','divorce_date_text','end_date_text','end_reason','status','confidence','review_status','note','created_at','updated_at'],
  parent_child_relations: ['id','parent_id','child_id','relation_type','start_date_text','end_date_text','confidence','review_status','note','created_at','updated_at'],
...
export function buildStandardCsvSetFiles(data: Omit<StandardCsvSetData, 'importBatches' | 'events'> & { events?: Event[] }): StandardCsvSetTextFiles {
  ...
  return {
    ...
    'unions.csv': csv(STANDARD_CSV_COLUMNS.unions, data.unions.map(u => ({ id:u.id, external_id:u.external_id ?? '', partner1_id:u.partner1_id, partner2_id:u.partner2_id ?? '', union_type:u.union_type, marriage_date_text:u.marriage_date_text ?? '', divorce_date_text:u.divorce_date_text ?? '', end_date_text:u.end_date_text ?? '', end_reason:u.end_reason ?? '', status:u.status ?? '', confidence:u.confidence ?? '', review_status:u.review_status ?? '', note:u.note ?? '', created_at:u.created_at, updated_at:u.updated_at }))),
    'parent_child_relations.csv': csv(STANDARD_CSV_COLUMNS.parent_child_relations, data.parentChildRelations.map(r => ({ id:r.id, parent_id:r.parent_id, child_id:r.child_id, relation_type:r.relation_type, start_date_text:r.start_date_text ?? '', end_date_text:r.end_date_text ?? '', confidence:r.confidence ?? '', review_status:r.review_status ?? '', note:r.note ?? '', created_at:r.created_at, updated_at:r.updated_at }))),
...
function req(row: Record<string,string>, field:string, file:string, rowNo:number, issues:ValidationIssue[]) { const value = v(row[field]).trim(); if (!value) issues.push({ severity:'error', code:'empty_required_value', message:`${file} ${rowNo}行目の必須列 ${field} が空欄です。`, row:rowNo, field }); return value; }
const REQUIRED_FILES = ['manifest.json','persons.csv','unions.csv','parent_child_relations.csv','sources.csv','citations.csv'] as const;
const REQUIRED_COLUMNS: Record<string, readonly string[]> = { 'persons.csv':['id','name'], 'unions.csv':['id','partner1_id'], 'parent_child_relations.csv':['id','parent_id','child_id','relation_type'], 'sources.csv':['id','source_type','title'], 'citations.csv':['id','source_id','target_type','target_id'], 'events.csv':['id','event_type','target_type','target_id'] };
const ENUMS: Record<string, Record<string, readonly string[]>> = {
  'persons.csv': { gender:['male','female','unknown','other'], confidence:['confirmed','likely','uncertain','disputed'], review_status:['unreviewed','reviewed','rejected'], is_living:['true','false','unknown'], privacy_level:['public','private','hidden'] },
  'unions.csv': { union_type:['marriage','partner','concubine','unknown','other'], end_reason:['divorce','death','unknown','other'], status:['married','divorced','widowed','ended','unknown'], confidence:['confirmed','likely','uncertain','disputed'], review_status:['unreviewed','reviewed','rejected'] },
  'parent_child_relations.csv': { relation_type:['biological','adoptive','special_adoptive','step','recognized','foster','unknown','disputed'], confidence:['confirmed','likely','uncertain','disputed'], review_status:['unreviewed','reviewed','rejected'] },
  'events.csv': { event_type:['birth','death','marriage','divorce','adoption','recognition','entry_registry','removal_registry','transfer_registry','name_change','residence','occupation','title','other'], target_type:['person','union','relation'], confidence:['confirmed','likely','uncertain','disputed'], review_status:['unreviewed','reviewed','rejected'] },
  'sources.csv': { source_type:['current_koseki','joseki','kaisei_genkoseki','family_register_copy','grave','temple_record','interview','book','article','website','photo','ai_generated','other'], privacy_level:['public','private','hidden'] },
  'citations.csv': { target_type:['person','event','union','relation','name','place'], confidence:['confirmed','likely','uncertain','disputed'] },
};
function headers(rows: Record<string,string>[]) { return new Set(rows.length ? Object.keys(rows[0]) : []); }
function checkRequiredColumns(file:string, rows:Record<string,string>[], issues:ValidationIssue[]) { if (!rows.length) return; const h=headers(rows); for (const c of REQUIRED_COLUMNS[file] ?? []) if (!h.has(c)) issues.push({ severity:'error', code:'missing_required_column', message:`${file} に必須列 ${c} がありません。`, field:c }); }
function checkDuplicateIds(file:string, rows:Record<string,string>[], issues:ValidationIssue[]) { const seen=new Map<string,number>(); rows.forEach((r,i)=>{ const id=v(r.id).trim(); if(!id) return; const row=i+2; const first=seen.get(id); if(first) issues.push({ severity:'error', code:'duplicate_id_in_file', message:`${file} の id ${id} が重複しています。`, row, field:'id', target_id:id, related_ids:[String(first)] }); else seen.set(id,row); }); }
function checkEnums(file:string, rows:Record<string,string>[], issues:ValidationIssue[]) { const defs=ENUMS[file] ?? {}; rows.forEach((r,i)=>{ for (const [field, allowed] of Object.entries(defs)) { const value=v(r[field]).trim(); if (value && !allowed.includes(value)) issues.push({ severity:'error', code: field === 'target_type' ? 'invalid_target_type' : 'invalid_enum_value', message:`${file} ${i+2}行目 ${field} の値 ${value} は不正です。`, row:i+2, field, target_id:value }); } }); }
function validateRows(file:string, parsed:{rows:Record<string,string>[]}, issues:ValidationIssue[]) { checkRequiredColumns(file, parsed.rows, issues); checkDuplicateIds(file, parsed.rows, issues); checkEnums(file, parsed.rows, issues); }
...
export function validateStandardCsvSet(files: StandardCsvSetTextFiles, options: ImportPreviewOptions = {}): StandardCsvSetPreview {
  ...
  if (!files['manifest.json']) issues.push({ severity:'error', code:'missing_manifest', message:'manifest.json がありません。' });
    ...
    try {
      const m = JSON.parse(files['manifest.json']) as { format?: string; version?: string; schema_version?: string; files?: unknown };
      if (m.format !== STANDARD_CSV_SET_FORMAT) issues.push({ severity:'error', code:'invalid_manifest_format', message:'manifest.json の format が Kakeizu Studio 標準CSVセットではありません。' });
      if (!m.version && !m.schema_version) issues.push({ severity:'warning', code:'unsupported_manifest_version', message:'manifest.json に version または schema_version がありません。' });
      if (Array.isArray(m.files)) for (const f of m.files) if (typeof f === 'string' && f !== 'events.csv' && !files[f]) issues.push({ severity:'error', code:'missing_manifest_file_entry', message:`manifest.json には ${f} が記載されていますが、実ファイルがありません。`, field:'files' });
    } catch { issues.push({ severity:'error', code:'invalid_manifest_json', message:'manifest.json をJSONとして読み取れません。' }); }
  }
  for (const f of REQUIRED_FILES) if (!files[f]) { issues.push({ severity:'error', code:'missing_required_file', message:`${f} が見つかりません。` }); if (f !== 'manifest.json') issues.push({ severity:'error', code:'missing_csv', message:`${f} がありません。` }); }
  const now = new Date().toISOString();
  const prs = files['persons.csv'] ? parseRows(files['persons.csv']) : { rows:[], errors:[] }; validateRows('persons.csv', prs, issues); prs.errors.forEach(e=>issues.push({severity:'error',code:'csv_parse_error',message:e.message,row:e.row}));
  ...
  const urs = files['unions.csv'] ? parseRows(files['unions.csv']) : { rows:[], errors:[] }; validateRows('unions.csv', urs, issues); urs.errors.forEach(e=>issues.push({severity:'error',code:'csv_parse_error',message:e.message,row:e.row})); const unions = urs.rows.map((r,i): Union => ({ id:req(r,'id','unions.csv',i+2,issues), external_id:opt(r.external_id), partner1_id:req(r,'partner1_id','unions.csv',i+2,issues), partner2_id:opt(r.partner2_id), union_type:(opt(r.union_type) as Union['union_type']) ?? 'unknown', marriage_date_text:opt(r.marriage_date_text) ?? opt(r.start_date), divorce_date_text:opt(r.divorce_date_text), end_date_text:opt(r.end_date_text) ?? opt(r.end_date), end_reason:opt(r.end_reason) as Union['end_reason'], status:opt(r.status) as Union['status'], confidence:opt(r.confidence) as Union['confidence'], review_status:opt(r.review_status) as Union['review_status'], note:opt(r.note), created_at:opt(r.created_at) ?? now, updated_at:opt(r.updated_at) ?? now }));
  const unionIds = new Set(unions.map(u=>u.id).filter(Boolean));
  const rrs = files['parent_child_relations.csv'] ? parseRows(files['parent_child_relations.csv']) : { rows:[], errors:[] }; validateRows('parent_child_relations.csv', rrs, issues); rrs.errors.forEach(e=>issues.push({severity:'error',code:'csv_parse_error',message:e.message,row:e.row})); const parentChildRelations = rrs.rows.map((r,i): ParentChildRelation => ({ id:req(r,'id','parent_child_relations.csv',i+2,issues), parent_id:req(r,'parent_id','parent_child_relations.csv',i+2,issues), child_id:req(r,'child_id','parent_child_relations.csv',i+2,issues), union_id:opt(r.union_id), relation_type:(opt(r.relation_type) as ParentChildRelation['relation_type']) ?? 'unknown', start_date_text:opt(r.start_date_text) ?? opt(r.start_date), end_date_text:opt(r.end_date_text) ?? opt(r.end_date), confidence:opt(r.confidence) as ParentChildRelation['confidence'], review_status:opt(r.review_status) as ParentChildRelation['review_status'], note:opt(r.note), created_at:opt(r.created_at) ?? now, updated_at:opt(r.updated_at) ?? now }));
  const relationIds = new Set(parentChildRelations.map(r=>r.id).filter(Boolean));
  const srs = files['sources.csv'] ? parseRows(files['sources.csv']) : { rows:[], errors:[] }; validateRows('sources.csv', srs, issues); srs.errors.forEach(e=>issues.push({severity:'error',code:'csv_parse_error',message:e.message,row:e.row})); const sources = srs.rows.map((r,i): Source => ({ id:req(r,'id','sources.csv',i+2,issues), external_id:opt(r.external_id), source_type:(opt(r.source_type) as Source['source_type']) ?? 'other', title:req(r,'title','sources.csv',i+2,issues), author_or_issuer:opt(r.author_or_issuer), issued_date_text:opt(r.issued_date_text), obtained_date:opt(r.obtained_date), repository:opt(r.repository), honseki_text:opt(r.honseki_text), head_of_registry:opt(r.head_of_registry), registry_type:opt(r.registry_type), source_text:opt(r.source_text), url:opt(r.url), privacy_level:opt(r.privacy_level) as Source['privacy_level'], note:opt(r.note), import_batch_id:opt(r.import_batch_id), created_at:opt(r.created_at) ?? now, updated_at:opt(r.updated_at) ?? now }));
  const sourceIds = new Set(sources.map(s=>s.id).filter(Boolean));
  const ers = files['events.csv'] ? parseRows(files['events.csv']) : { rows:[], errors:[] }; validateRows('events.csv', ers, issues); ers.errors.forEach(e=>issues.push({severity:'error',code:'csv_parse_error',message:e.message,row:e.row})); const events = ers.rows.map((r,i): Event => ({ id:req(r,'id','events.csv',i+2,issues), external_id:opt(r.external_id), event_type:(req(r,'event_type','events.csv',i+2,issues) as Event['event_type']), target_type:(req(r,'target_type','events.csv',i+2,issues) as Event['target_type']), target_id:req(r,'target_id','events.csv',i+2,issues), date_text:opt(r.date_text), date_from:opt(r.date_from), date_to:opt(r.date_to), place_text:opt(r.place_text), description:opt(r.description), confidence:opt(r.confidence) as Event['confidence'], review_status:opt(r.review_status) as Event['review_status'], note:opt(r.note), import_batch_id:opt(r.import_batch_id), created_at:opt(r.created_at) ?? now, updated_at:opt(r.updated_at) ?? now }));
  const eventIds = new Set(events.map(e=>e.id).filter(Boolean));
  const crs = files['citations.csv'] ? parseRows(files['citations.csv']) : { rows:[], errors:[] }; validateRows('citations.csv', crs, issues); crs.errors.forEach(e=>issues.push({severity:'error',code:'csv_parse_error',message:e.message,row:e.row})); const citations = crs.rows.map((r,i): Citation => ({ id:req(r,'id','citations.csv',i+2,issues), external_id:opt(r.external_id), source_id:req(r,'source_id','citations.csv',i+2,issues), target_type:(req(r,'target_type','citations.csv',i+2,issues) as Citation['target_type']), target_id:req(r,'target_id','citations.csv',i+2,issues), page_or_location:opt(r.page_or_location), quote_text:opt(r.quote_text), interpretation:opt(r.interpretation), confidence:opt(r.confidence) as Citation['confidence'], note:opt(r.note), import_batch_id:opt(r.import_batch_id), created_at:opt(r.created_at) ?? now, updated_at:opt(r.updated_at) ?? now }));
  unions.forEach((u,i)=>{ if(u.partner1_id && !personIds.has(u.partner1_id)) issues.push({severity:'error',code:'missing_person_ref',message:`unions.csv ${i+2}行目 partner1_id: ${u.partner1_id} が見つかりません。`,row:i+2,field:'partner1_id',target_type:'union',target_id:u.id}); if(u.partner2_id && !personIds.has(u.partner2_id)) issues.push({severity:'error',code:'missing_person_ref',message:`unions.csv ${i+2}行目 partner2_id: ${u.partner2_id} が見つかりません。`,row:i+2,field:'partner2_id',target_type:'union',target_id:u.id}); });
  parentChildRelations.forEach((r,i)=>{ if(r.parent_id && !personIds.has(r.parent_id)) issues.push({severity:'error',code:'missing_person_ref',message:`parent_child_relations.csv ${i+2}行目 parent_id: ${r.parent_id} が見つかりません。`,row:i+2,field:'parent_id',target_type:'relation',target_id:r.id}); if(r.child_id && !personIds.has(r.child_id)) issues.push({severity:'error',code:'missing_person_ref',message:`parent_child_relations.csv ${i+2}行目 child_id: ${r.child_id} が見つかりません。`,row:i+2,field:'child_id',target_type:'relation',target_id:r.id}); if(r.union_id && !unionIds.has(r.union_id)) issues.push({severity:'error',code:'missing_union_ref',message:`parent_child_relations.csv ${i+2}行目 union_id: ${r.union_id} が見つかりません。`,row:i+2,field:'union_id',target_type:'relation',target_id:r.id}); });
  events.forEach((e,i)=>{ if(e.target_type==='person' && !personIds.has(e.target_id)) issues.push({severity:'error',code:'missing_person_ref',message:`events.csv ${i+2}行目 target_id: ${e.target_id} が見つかりません。`,row:i+2,field:'target_id',target_type:'event',target_id:e.id}); else if(e.target_type==='union' && !unionIds.has(e.target_id)) issues.push({severity:'error',code:'missing_union_ref',message:`events.csv ${i+2}行目 target_id: ${e.target_id} が見つかりません。`,row:i+2,field:'target_id',target_type:'event',target_id:e.id}); else if(e.target_type==='relation' && !relationIds.has(e.target_id)) issues.push({severity:'error',code:'missing_relation_ref',message:`events.csv ${i+2}行目 target_id: ${e.target_id} が見つかりません。`,row:i+2,field:'target_id',target_type:'event',target_id:e.id}); });
  citations.forEach((c,i)=>{ if(c.source_id && !sourceIds.has(c.source_id)) issues.push({severity:'error',code:'missing_source_ref',message:`citations.csv ${i+2}行目 source_id: ${c.source_id} が見つかりません。`,row:i+2,field:'source_id',target_type:'citation',target_id:c.id}); if(c.target_type==='person' && !personIds.has(c.target_id)) issues.push({severity:'error',code:'missing_person_ref',message:`citations.csv ${i+2}行目 target_id: ${c.target_id} が見つかりません。`,row:i+2,field:'target_id',target_type:'citation',target_id:c.id}); else if(c.target_type==='union' && !unionIds.has(c.target_id)) issues.push({severity:'error',code:'missing_union_ref',message:`citations.csv ${i+2}行目 target_id: ${c.target_id} が見つかりません。`,row:i+2,field:'target_id',target_type:'citation',target_id:c.id}); else if(c.target_type==='relation' && !relationIds.has(c.target_id)) issues.push({severity:'error',code:'missing_relation_ref',message:`citations.csv ${i+2}行目 target_id: ${c.target_id} が見つかりません。`,row:i+2,field:'target_id',target_type:'citation',target_id:c.id}); else if(c.target_type==='event' && !eventIds.has(c.target_id)) issues.push({severity:'error',code:'missing_event_ref',message:`citations.csv ${i+2}行目 target_id: ${c.target_id} が見つかりません。`,row:i+2,field:'target_id',target_type:'citation',target_id:c.id}); else if(['name','place'].includes(c.target_type)) issues.push({severity:'warning',code:'unsupported_target_type',message:`${c.target_type} は将来用target_typeとして読み込みますが、参照先データは未実装です。`,row:i+2,field:'target_type',target_type:'citation',target_id:c.id}); });

  for (const issue of [...issues]) {
    const mapped = issue.code === 'missing_person_ref' ? 'unresolved_person_reference' : issue.code === 'missing_union_ref' ? 'unresolved_union_reference' : issue.code === 'missing_relation_ref' ? 'unresolved_relation_reference' : issue.code === 'missing_event_ref' ? 'unresolved_event_reference' : issue.code === 'missing_source_ref' ? 'unresolved_source_reference' : undefined;
    if (mapped) issues.push({ ...issue, code: mapped });
  }
