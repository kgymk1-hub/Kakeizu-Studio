...
const steps = ['CSV入力', '列マッピング', 'プレビュー', '検証', '反映'];
...
export function ImportPolicySelector({ importPolicy, onChange, name = 'import-policy' }: { importPolicy: ImportPolicy; onChange: (importPolicy: ImportPolicy) => void; name?: string }) {
  ...
  return <div className="import-policy"><fieldset><legend>インポート方式</legend>{importPolicyOptions.map((option)=><label key={option.value} className="import-policy-option"><input type="radio" name={name} value={option.value} checked={importPolicy===option.value} onChange={()=>onChange(option.value)} /><span><strong>{option.label}</strong> <small>{option.status === 'available' ? '実行可能' : 'プレビューのみ'}</small><br/><span>{option.description}</span></span></label>)}</fieldset><p><strong>インポート方式：{selected.label}</strong></p><p>{selected.status === 'available' ? selected.description : 'この方式は後続フェーズで実装予定です。現在はプレビューのみ確認できます。'}</p></div>;
}
...
function MatchPreview({ preview }: { preview: ImportPreviewResult }) {
  ...
  return <div className="match-preview"><h4>external_id照合結果</h4><MetricPills items={[{ key: 'new-items', content: <>新規候補: {matchSummary.newItems}件</> }, { key: 'matched-existing', content: <>既存一致候補: {matchSummary.matchedExisting}件</> }, { key: 'duplicate-in-import', content: <>CSV内重複: {matchSummary.duplicateInImport}件</> }, { key: 'missing-external-id', content: <>external_idなし: {matchSummary.missingExternalId}件</> }, { key: 'unknown', content: <>不明: {matchSummary.unknown}件</> }]} /><h4>このインポート方式での予定処理</h4><MetricPills items={[{ key: 'create', content: <>作成候補: {policyPlan.create}件</> }, { key: 'update', content: <>更新候補: {policyPlan.update}件</> }, { key: 'skip', content: <>スキップ候補: {policyPlan.skip}件</> }, { key: 'add-as-new', content: <>別ID追加候補: {policyPlan.addAsNew}件</> }, { key: 'replace', content: <>全置換対象: {policyPlan.replace}件</> }, { key: 'blocked', content: <>保留/要確認: {policyPlan.blocked}件</> }]} /><h4>照合詳細</h4><ul className="compact-list">{preview.matches.slice(0, 10).map((m, idx)=><li key={`${m.entityType}-${m.importId ?? m.externalId ?? idx}`}>{m.fileName && `${m.fileName}: `}{m.externalId ?? m.importId ?? '(external_idなし)'} / {m.displayName ?? m.importId ?? '-'} / {label[m.status]}{m.existingId ? `: ${m.existingId}` : ''}</li>)}</ul>{preview.matches.length > 10 && <p className="help-text">照合詳細は先頭10件のみ表示しています。</p>}</div>;
}
...
function IssueList({ issues }: { issues: { severity: string; code: string; message: string; rowNumber?: number; fileName?: string; field?: string; targetType?: string; targetId?: string }[] }) {
  if (issues.length === 0) return <EmptyState>問題はありません。</EmptyState>;
...
export function CsvImport({ onImported, existingData }: { onImported: (data: NormalizedFamilyData, preview: ImportPreviewResult) => Promise<ImportReport | false> | ImportReport | false; existingData?: ExistingImportContext }) {
  ...
  return <section className="panel import-panel"><h2>CSVインポート</h2>
    ...
    {step >= 2 && analysis && <div><h3>プレビュー</h3><ImportPolicySelector name="simple-import-policy" importPolicy={importPolicy} onChange={setImportPolicy} /><PlaceholderPersonPolicySelector name="simple-placeholder-policy-preview" policy={placeholderPersonPolicy} onChange={setPlaceholderPersonPolicy} /><MetricPills items={[{ key: 'total-rows', content: <>総行数: {analysis.preview.summary.totalRows}</> }, { key: 'valid-rows', content: <>正常行: {analysis.preview.summary.validRows}</> }, { key: 'warning-rows', content: <>警告行: {analysis.preview.summary.warningRows}</> }, { key: 'error-rows', content: <>エラー行: {analysis.preview.summary.errorRows}</> }, { key: 'warning-issues', content: <>warning: {analysis.preview.summary.warningIssues}</> }, { key: 'error-issues', content: <>error: {analysis.preview.summary.errorIssues}</> }]} /><MetricPills items={[{ key: 'persons', content: <>Person: {analysis.preview.summary.plannedCreate.persons}件</> }, { key: 'unions', content: <>Union: {analysis.preview.summary.plannedCreate.unions}件</> }, { key: 'relations', content: <>ParentChildRelation: {analysis.preview.summary.plannedCreate.relations}件</> }, { key: 'events', content: <>Event: {analysis.preview.summary.plannedCreate.events}件</> }, { key: 'sources', content: <>Source: {analysis.preview.summary.plannedCreate.sources}件</> }, { key: 'citations', content: <>Citation: {analysis.preview.summary.plannedCreate.citations}件</> }]} /><MatchPreview preview={analysis.preview} /><UnresolvedReferencePreview preview={analysis.preview} />{!analysis.preview.canImport ? <p className="error">{analysis.preview.summary.importPolicyStatus === 'preview_only' ? 'このインポート方式は現在プレビューのみ対応です。実行はできません。' : 'エラーがあるため反映不可です。'}</p> : analysis.preview.hasWarnings ? <p className="warning">警告があります。内容を確認すれば反映可能です。</p> : <p className="success">エラー・警告なし。反映可能です。</p>}<div className="preview-scroll"><table className="preview-table"><thead><tr><th>行番号</th>{previewColumns.map((c)=><th key={c}>{c}</th>)}<th>判定結果</th></tr></thead><tbody>{analysis.parsedRows.map((row, index)=>{ const rowIssues = analysis.result.issues.filter((issue)=>issue.row===index+2 || issue.external_id===row.person_id); const hasError = rowIssues.some((issue)=>issue.severity==='error'); const hasWarning = rowIssues.some((issue)=>issue.severity==='warning'); return <tr key={`${row.person_id}-${index}`} className={hasError?'error-row':hasWarning?'warning-row':''}><td>{index+2}</td>{previewColumns.map((c)=><td key={c}>{String(row[c] ?? '')}</td>)}<td>{hasError?'エラーあり':hasWarning?'警告あり':'正常'}</td></tr>; })}</tbody></table></div><button onClick={()=>setStep(3)}>検証結果へ進む</button></div>}

    {step >= 3 && analysis && <div className="validation-summary"><h3>検証結果確認</h3><p className="help-text">errorは修正が必要な問題で、反映をブロックします。warningは未登録ID参照などの確認事項で、確認後に反映できます。</p><p>総行数 {analysis.preview.summary.totalRows}件 / 正常行 {analysis.preview.summary.validRows}件 / 警告行 {analysis.preview.summary.warningRows}件 / エラー行 {analysis.preview.summary.errorRows}件 / warning {analysis.preview.summary.warningIssues}件 / error {analysis.preview.summary.errorIssues}件</p><p>反映予定: Person {analysis.preview.summary.plannedCreate.persons}件 / Union {analysis.preview.summary.plannedCreate.unions}件 / ParentChildRelation {analysis.preview.summary.plannedCreate.relations}件 / Event {analysis.preview.summary.plannedCreate.events}件 / Source {analysis.preview.summary.plannedCreate.sources}件 / Citation {analysis.preview.summary.plannedCreate.citations}件</p><MatchPreview preview={analysis.preview} /><UnresolvedReferencePreview preview={analysis.preview} /><p>未登録人物IDの参照件数 {analysis.summary.placeholderPersonCount}件 / 自動補完された配偶者関係 {analysis.summary.autoCompletedSpouseCount}件</p><ImportPolicySelector name="simple-import-policy" importPolicy={importPolicy} onChange={setImportPolicy} /><PlaceholderPersonPolicySelector name="simple-placeholder-policy-validation" policy={placeholderPersonPolicy} onChange={setPlaceholderPersonPolicy} /><p className="warning"><strong>インポート方式：{getImportPolicyOption(importPolicy).label}</strong>。{importPolicy === 'replace_all' ? 'インポートを実行すると、現在の家系図データと資料・出典は、このCSVの内容で置き換えられます（CSVには資料・出典を含めないため既存の資料・出典は削除されます）。' : 'このインポート方式は現在プレビューのみ対応です。実行はできません。'}</p>{!analysis.preview.canImport ? <p className="error">{analysis.preview.summary.importPolicyStatus === 'preview_only' ? 'このインポート方式は現在プレビューのみ対応です。実行はできません。' : 'エラーがあるため反映できません。'} <button onClick={()=>issueRef.current?.scrollIntoView({behavior:'smooth'})}>エラー一覧へ</button></p> : analysis.summary.warningCount>0 ? <p className="warning">警告がありますが反映可能です。</p> : <p>エラー・警告なし。反映可能です。</p>}<div ref={issueRef} className="issue-box"><IssueList issues={analysis.preview.issues} /></div><button className="primary" disabled={!analysis.preview.canImport} onClick={runImport}>家系図・資料・出典を置き換えて反映</button></div>}

    {lastReport && <div className="import-report"><h3>インポート結果レポート</h3><p><strong>インポート結果:</strong> {lastReport.status === 'success_with_warnings' ? 'warning付き成功' : lastReport.status === 'success' ? '成功' : lastReport.status}</p><p><strong>インポート元:</strong> {lastReport.mode === 'simple_csv' ? 'かんたんCSV' : '標準CSVセット'} / <strong>実行日時:</strong> {new Date(lastReport.createdAt).toLocaleString()}</p><MetricPills items={[{ key: 'persons', content: <>Person {lastReport.importedCounts.persons}</> }, { key: 'unions', content: <>Union {lastReport.importedCounts.unions}</> }, { key: 'relations', content: <>Relation {lastReport.importedCounts.relations}</> }, { key: 'sources', content: <>Source {lastReport.importedCounts.sources}</> }, { key: 'citations', content: <>Citation {lastReport.importedCounts.citations}</> }, { key: 'events', content: <>Event {lastReport.importedCounts.events}</> }]} /><MetricPills items={[{ key: 'warning', content: <>warning {lastReport.issueSummary.warningIssues}</> }, { key: 'error', content: <>error {lastReport.issueSummary.errorIssues}</> }, { key: 'total', content: <>total {lastReport.issueSummary.totalIssues}</> }]} /><h4>次に確認すること</h4><ul className="compact-list">{lastReport.nextActions.map((action)=><li key={action}>{action}</li>)}</ul></div>}
