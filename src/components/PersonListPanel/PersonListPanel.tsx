...
export function PersonListPanel({ persons, citations, selectedPersonId, onSelectTarget }: Props) {
  ...
  return <section className="panel list-panel person-list-panel" aria-labelledby="person-list-heading">
    <h2 id="person-list-heading">人物一覧</h2>
    <div className="list-panel-controls person-list-controls">
    ...
    {filteredPersons.length === 0 ? <EmptyState className="notice list-panel-empty">条件に一致する人物がありません。</EmptyState> : <ul className="list-card-list person-list-cards">
      {filteredPersons.map((person) => {
        ...
        return <li key={person.id}>
          <button type="button" className={`list-card list-card-clickable person-list-card ${person.id === selectedPersonId ? 'selected-list' : ''}`} onClick={() => onSelectTarget({ target_type: 'person', target_id: person.id })}>
            ...
            <span className="list-card-meta person-card-meta"><span>{person.rank_title || person.occupation || '称号・職業未入力'}</span><span>{person.confidence ?? 'confirmed'} / {person.review_status ?? 'unreviewed'} / {cited ? '出典あり' : '出典なし'}</span></span>
