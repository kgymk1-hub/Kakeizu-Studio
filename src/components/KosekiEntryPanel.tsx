...
const additionalEventOptions: { value: EventType; label: string }[] = [
  { value: 'marriage', label: '婚姻 marriage' },
  { value: 'divorce', label: '離婚 divorce' },
  { value: 'adoption', label: '養子縁組 adoption' },
  { value: 'recognition', label: '認知 recognition' },
  { value: 'entry_registry', label: '入籍 entry_registry' },
  { value: 'removal_registry', label: '除籍 removal_registry' },
  { value: 'transfer_registry', label: '転籍 transfer_registry' },
  { value: 'name_change', label: '氏名変更 name_change' },
  { value: 'residence', label: '居住 residence' },
  { value: 'occupation', label: '職業 occupation' },
  { value: 'title', label: '称号・肩書 title' },
...
export function KosekiEntryPanel({ persons, sources, citations, relations, unions, events, onCreateSource, onApply }: Props) {
  ...
  return <section className="panel koseki-entry"><h2>戸籍入力モード</h2>
    <p className="help-text">戸籍・除籍・改製原戸籍などの資料を見ながら手入力する支援モードです。OCRやAI読み取りは行わず、人物と任意作成した出生・死亡の出来事（Event）・追加の出来事に出典（Citation）を紐づけます。</p>
    <p className="help-text">親子関係・配偶者関係への出典付与は未対応です。既存人物更新では、空欄の項目は既存値を保持します。出生の出来事 / 死亡の出来事のチェックがONでも、対応する日付テキストが空の場合は出来事を作成しません。追加の出来事は人物に紐づく出来事として記録し、現時点では家系図の関係や人物基本情報へ自動反映しません。</p>
    ...
    <details><summary>簡易資料（Source）を作成</summary><form className="stack-form" onSubmit={(e)=>{e.preventDefault(); void makeSource(e.currentTarget);}}>
    ...
    <form key={`${mode}-${personId}`} className="stack-form" onSubmit={(e)=>{e.preventDefault(); void submitPerson(e.currentTarget);}}>
      ...
      <label><input type="checkbox" name="createBirthEvent" /> 出生の出来事を作成する</label><label><input type="checkbox" name="createDeathEvent" /> 死亡の出来事を作成する</label>
      <fieldset><legend>追加の出来事</legend><p className="help-text">出生・死亡以外の戸籍上の出来事を、選択中人物に出来事として追加します。家系図や人物基本情報への自動反映はまだ行いません。</p><p className="help-text">対象人物: 新規人物作成時は作成される人物、既存人物更新時は選択中の既存人物です。追加の出来事にも選択中戸籍資料への出典が付きます。</p><label>出来事種別<select name="additionalEventType" defaultValue=""><option value="">選択しない</option>{additionalEventOptions.map((option)=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>日付テキスト<input name="additionalEventDateText" /></label><label>場所<input name="additionalEventPlaceText" /></label><label>説明<textarea name="additionalEventDescription" /></label><label>メモ<textarea name="additionalEventNote" /></label><label>確度<select name="additionalEventConfidence" defaultValue=""><option value="">人物フォームの確度を使う</option><option value="confirmed">confirmed</option><option value="likely">likely</option><option value="uncertain">uncertain</option><option value="disputed">disputed</option></select></label></fieldset>
