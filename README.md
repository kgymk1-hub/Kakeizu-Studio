...
v0.9.0では、Name / Placeモデル最小版を追加し、PersonとName、Event / SourceとPlaceを最小限連携できるようにしました。NameはPerson表示名の完全置換ではなく、別名・旧名・通称などの追加情報として扱います。Placeは`Event.place_text`や`Source.honseki_text` / `Source.repository`の完全置換ではなく、場所の正規化・再利用候補として扱います。また、JSON backup `schema_version`を`1.4`に更新し、Name / PlaceをJSONバックアップで保存・復元できるようにしました。

v0.9第1〜第9フェーズと補完フェーズの内容は、正式版の実装内容として以下のとおり整理しています。

- Name / Place現状棚卸し: 既存のPerson表示名、Event場所テキスト、Source本籍・所蔵情報、Citation target_typeの状態を`docs/name_place_audit_v0.9.md`に整理しました。
- Nameモデル最小版: `NameType` / `Name`を追加し、別名・旧名・通称などをPersonへ横付けできる土台を追加しました。
- PersonとNameの最小連携: Person詳細でName一覧を確認し、追加・編集・削除できる最小導線を追加しました。
- Placeモデル最小版: `PlaceType` / `Place`を追加し、場所の正規化・再利用候補を保持できる土台を追加しました。
- Event / Source とPlaceの最小連携: `Event.place_id` / `Source.place_id`を任意Place参照として保存・復元できるようにしました。
- Name / Place一覧・検索最小版: Name / Place一覧と検索を追加し、Nameから関連Personへ移動できるようにしました。
- Citation target_type name / place の準備: `target_type=name/place`を安全表示・参照検証の対象にしました。
- JSONバックアップ・DB互換性確認: JSON backup `schema_version: "1.4"`で`names` / `places`を保存・復元し、JSON 1.3以前の復元互換を維持しました。
- 補完フェーズ：Place編集導線と参照検証: Place追加・編集・削除、Place削除時の`Event.place_id` / `Source.place_id`安全処理、参照切れ検証を整理しました。
- v0.9全体仕上げ確認: Name / Placeが既存フィールドを置き換えない境界、標準CSVセットの非変更、既存v0.7/v0.8機能の維持を確認しました。

### v0.9.0 方針

- NameはPerson表示名の完全置換ではありません。
- `Person.display_name` / `family_name` / `given_name` / `birth_family_name` 等は維持します。
- Name編集でPerson本体を勝手に上書きしません。
- Placeは既存場所テキストの完全置換ではありません。
- `Event.place_text` は維持します。
- `Source.honseki_text` / `Source.repository` は維持します。
- Place編集で`Event.place_text`や`Source.honseki_text` / `Source.repository`を勝手に上書きしません。
- JSONバックアップではName / Placeを保存・復元できます。
- JSON backup `schema_version` は `1.4` のまま維持します。
- Dexie schemaは既存 `version(1)`〜`version(5)` を維持し、`version(6)` は追加しません。
- 標準CSVセットには `names.csv` / `places.csv` をまだ追加していません。
- 標準CSVセット経由ではName / Place実体をまだ入出力しません。
...
- [ ] Nameモデル最小版がある。
- [ ] NameTypeがある。
- [ ] Person詳細でName一覧を確認できる。
- [ ] Person詳細でNameを追加できる。
- [ ] Person詳細でNameを編集できる。
- [ ] Person詳細でNameを削除できる。
- [ ] Name編集でPerson本体を勝手に上書きしない。
- [ ] Name一覧・検索が使える。
- [ ] Nameクリックで関連Personへ移動できる。
- [ ] Placeモデル最小版がある。
- [ ] PlaceTypeがある。
- [ ] Place一覧・検索が使える。
- [ ] Placeを追加できる。
- [ ] Placeを編集できる。
- [ ] Placeを削除できる。
- [ ] Place削除時にEvent / Source本体を削除しない。
- [ ] Place削除時にEvent.place_id / Source.place_idを安全に扱う。
- [ ] Event.place_idが任意Place参照として扱われる。
- [ ] Event.place_textは維持される。
- [ ] Source.place_idが任意Place参照として扱われる。
- [ ] Source.honseki_text / repositoryは維持される。
- [ ] Citation target_type=name/place を安全に表示できる。
- [ ] name/place Citationの参照切れを検証できる。
- [ ] Event.place_id / Source.place_idの参照切れを検証できる。
- [ ] JSON backup schema_version 1.4 でnames / placesが保存される。
- [ ] JSON 1.3以前の復元互換が維持される。
- [ ] Project / settings補完を壊していない。
- [ ] Dexie schema version(5) が追加されている。
- [ ] 標準CSVセット構造が変わっていない。
- [ ] names.csv / places.csv を標準CSVセットに追加していない。
- [ ] 既存のCSVインポートv0.7機能が壊れていない。
- [ ] 既存のProject / settings v0.8機能が壊れていない。
- [ ] 既存の人物一覧・Event一覧・Source/Citation一覧が壊れていない。
- [ ] 家系図表示が壊れていない。
- [ ] PNG / PDF / SVG出力が壊れていない。
- [ ] GitHub Pages確認URL <https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.9.0> で公開版を確認できる。
...
- NameとPerson表示名の完全同期。
- Placeと`Event.place_text`の完全同期。
- Placeと`Source.honseki_text` / `Source.repository`の完全同期。
- 地図表示。
- 緯度経度管理。
- 地名階層の本格管理。
- 本格ソート。
...
JSONバックアップの現行出力は`schema_version: "1.4"`です。schema 1.0〜1.4の代表fixtureについて、復元、不足配列・Project / settingsの補完、schema 1.4への再出力を自動テストで確認しています。代表fixtureは過去の全バックアップ出力パターンを完全に再現または保証するものではありません。「JSONバックアップ復元」は現在データを全置換します。復元前に、現在の作業データをJSONバックアップとして保存することを推奨します。
...
- CSVインポートの実保存は`replace_all`のみです。`append_new` / `update_by_external_id` / `skip_existing` / `add_as_new_ids`は`preview_only`であり、差分保存は行いません。
- 「JSONバックアップ復元」は現在データの全置換として動作するため、復元前バックアップを推奨します。
- Projectは単一default project相当です。本格的な複数Project切替や、Person等のデータ分離はありません。
- Name / Placeは横付けの最小版です。既存の表示名・場所文字列を置き換えず、自動同期もしません。
- 標準CSVセットには`names.csv` / `places.csv` / `media.csv`を追加しておらず、settingsにも対応していません。
- 公開用CSV / 公開用JSONはありません。通常JSONには公開用マスクを適用しません。
...
- JSONバックアップ出力・復元（現行出力はschema 1.4。schema 1.0〜1.4代表fixtureの復元、不足配列・settings補完、schema 1.4への再出力を自動テストで確認。代表fixtureは過去の全バックアップ出力パターンを完全に再現または保証するものではありません）
...
- 「JSONバックアップ」で現在の全データをアプリ内部形式のJSONとして出力します。
- 「JSONバックアップ復元」でJSONを復元します。復元すると現在データは全置換されるため、復元前に現在データを「JSONバックアップ」で保存することを推奨します。
- 現行出力は `schema_version: "1.4"` です。復元対応は schema 1.0〜1.4 です。
- 旧schema復元時は、不足する配列と Project / settings が補完されます。旧schemaを再出力すると、`schema_version: "1.4"` と新しい `exported_at` になります。
- Name / Place はJSONで保存・復元されます。
- Media はJSONバックアップに含まれません。
- 通常JSONには公開用マスクを適用しません。公開用に秘匿・加工したい場合は、別途公開向けデータを準備してください。
...
JSONバックアップはアプリ内部形式の完全バックアップです。現行出力は `schema_version: "1.4"` です。

主な保存対象は、`persons` / `unions` / `parent_child_relations` / `import_batches` / `sources` / `citations` / `events` / `projects` / `view_settings` / `export_settings` / `privacy_settings` / `names` / `places` です。Media は対象外です。

復元対応は schema 1.0〜1.4 です。旧schema復元時は、不足配列と Project / settings を補完します。再出力時は `schema_version: "1.4"` と新しい `exported_at` になります。

schema 1.0〜1.4の確認は代表fixtureによるものであり、過去の全バックアップ形式や全フィールド構成を完全保証するものではありません。互換性確認の詳細は `docs/backup_compatibility_v1.0.md` と `samples/compatibility/` を参照してください。
...
- 公開ページが真っ白画面になる場合は、以下を確認してください。
  ...
  - ブラウザやService Workerのキャッシュを避けるため、`https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.9.0` のようにクエリを付けて開く。

## 操作・データ上の補足制限

詳細は `docs/known_limitations_v1.0.md` も参照してください。
...
- Timeline表示と家系図ノード上のEvent表示は未対応です。
...
- Citation target_type=name/placeは安全表示・検証に対応しています。ただし、Citation編集UIでName / Placeを本格選択する機能は未対応です。
...
v1.1以降の候補は以下です。Name / Place最小版は実装済みであり、今後は本格管理や選択UIの拡張を検討します。

- 表形式入力・一括編集
- 本格的な複数Project
- 複数ビュー
- Name / Placeの本格管理とCitation選択UI
- Media / 添付ファイル
- 和暦本格解析
- GEDCOM
- 親等計算
