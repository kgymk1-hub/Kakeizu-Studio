...
- 補完フェーズ：Place追加・編集・削除の最小導線、Event / Source place_id参照切れ確認、標準CSVセットでのName / Place境界整理
- 第9フェーズ：v0.9全体仕上げ確認
...
- `Source.honseki_text` / `Source.repository` は引き続き維持し、`Source.place_id` は任意のPlace参照として保存・復元します。
- Name / Placeは既存フィールドを置き換えず、横に追加する最小版です。
- JSON backup `schema_version` は `1.4` とし、Name / Placeを保存・復元します。`1.3` 以前の復元互換も維持します。
- Dexie schema versionを `5` に上げる場合も、既存 `version(1)`〜`version(4)` を維持します。
- 標準CSVセット構造は今回は変更せず、`names.csv` / `places.csv` は追加しません。標準CSVセット経由ではName / Place実体をまだ入出力しません。
- `citations.csv` の `target_type=name/place` は安全表示・検証対象ですが、標準CSVセット内にName / Place実体CSVがないため、実体復元は将来対応です。
- Place削除時はPlace本体を削除し、そのPlaceを参照しているEvent / Sourceの `place_id` をクリアします。Event本体、Source本体、`Event.place_text`、`Source.honseki_text`、`Source.repository` は削除・同期上書きしません。
- `Event.place_id` / `Source.place_id` が参照切れの場合はValidationでwarningとして検出し、表示は落ちない方針です。


### v0.9時点の実装範囲と未対応事項

- Nameは、別名・旧名・通称などをPersonへ横付けする追加情報です。`Person.display_name`、`family_name`、`given_name`、`birth_family_name` などの人物基本情報を置き換えたり、Name編集で自動上書きしたりしません。
- Person詳細の「名前・別名」では、対象Personに紐づくNameの追加・編集・削除を行えます。Name削除はPerson本体を削除しません。
- Placeは、場所の正規化候補・再利用候補を扱う追加モデルです。`Event.place_text`、`Source.honseki_text`、`Source.repository` を置き換えたり、自動同期したりしません。
- Place追加・編集・削除はName / Place一覧から行えます。Place削除時は、そのPlaceを参照している `Event.place_id` / `Source.place_id` をクリアし、Event本体・Source本体・既存テキスト項目は維持します。
- `Source.place_id` は型・DB・JSONで保持する任意Place参照です。v0.9時点ではSource編集UIでのPlace選択は将来対応です。
- Citation `target_type=name/place` はSource / Citation一覧とValidationで安全表示・参照検証の対象です。Citation編集UIでのname/place本格選択は将来対応です。
- JSONバックアップは `schema_version: "1.4"` に上げ、Name / Place実体を `names` / `places` として保存・復元します。`1.3` 以前のJSONでは `names` / `places` を空配列として補完し、Project / settingsを含む既存補完も維持します。
- Dexie schema `version(5)` は、既存 `version(1)`〜`version(4)` を維持したまま `names` / `places` テーブルと `Event.place_id` / `Source.place_id` 保存に対応するための追加です。
- 標準CSVセットには `names.csv` / `places.csv` を追加していません。そのため、標準CSVセット経由ではName / Place実体は入出力しません。`citations.csv` の `target_type=name/place` は安全表示・検証対象ですが、標準CSVセットでName / Place実体を復元する機能は将来対応です。
