...
> v0.4.0では、検証エンジン、データ検証結果パネル、検証結果フィルタ、4桁西暦年ベースの日付・年齢チェックに対応しました。v0.3.0の関係単位Citation UI / 関係削除UI / 関係編集UIを維持しつつ、現在のアプリデータを都度検証できます。

## 公開URL

- GitHub Pages: <https://kgymk1-hub.github.io/Kakeizu-Studio/>
- v0.4.0 キャッシュ回避確認: <https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.4.0>
...
## v0.5 development: 関係線の表示強化

v0.5 development の第2フェーズでは、人物ノードではなく既存SVG家系図ビュー上の関係線を強化しています。package.json の version は `0.4.0` のまま変更しておりません。

- ParentChildRelation の `relation_type` に応じて親子線の線種を変えます。
- 実親子は実線、養親子・特別養親子・認知は破線系、継親子・養育は点線系、不明は薄い線、異説ありは警告色の線として見分けやすくしています。
...
- Unionノードにも、離婚、死別、終了済み、不明、要確認、異説ありを控えめに反映します。
...
## v0.5 development: SVG出力最小版

v0.5 development の第4フェーズでは、既存のSVG家系図表示を活かして、表示中の家系図を `.svg` ファイルとして保存できる最小版のSVG出力を追加しています。package.json の version は `0.4.0` のまま変更していません。

- PNG/PDF出力ボタンと同じヘッダーの出力導線に「SVG出力」ボタンを追加しました。
- 表示中の家系図領域を `kakeizu.svg` として保存できます。
- SVG出力には、出力タイトル、家系図SVG本体、凡例、背景スタイルを含めます。
- 家系図操作ツールバー、表示密度セレクト、出力用見た目設定UI、PNG/PDF/SVG出力ボタンなどの操作UIは出力対象から除外します。
- 背景は `white` では白い背景rect、`soft` では淡色背景rect、`transparent` ではSVG背景を透明として出力します。
- PNG/PDF出力導線は既存どおり維持し、同じ表示中の家系図領域を使う構成のままです。
- SVG出力は最小版のため、外部CSSやブラウザごとの差異を含む完全な印刷品質再現には制限があります。家系図表示に必要な最小限のstyleはSVG内へ埋め込んでいます。
- 本格的なSVGテンプレート、用紙サイズ指定、ページ分割、出典一覧付き出力、高品質印刷用SVGの詳細調整は未対応です。
- 新しいDBテーブル、JSON schema_version 変更、Project / ViewSetting / ExportSetting への永続化は行っていません。
- package.json の version は変更していません。

## v0.4.0 確認用URL

- 公開URL: <https://kgymk1-hub.github.io/Kakeizu-Studio/>
- キャッシュ回避確認: <https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.4.0>
...
JSONバックアップはアプリ内部形式そのものです。v0.4.0では `schema_version` は必要がなければ `1.2` のままで、`persons` / `unions` / `parent_child_relations` / `import_batches` / `sources` / `citations` / `events` をまとめて保持します。旧 `1.0` / `1.1` JSONで `events` が存在しない場合は空配列として扱います。普段のバックアップや、Kakeizu Studio間で完全に復元したい場合に向いています。
...
- 公開ページが真っ白画面になる場合は、以下を確認してください。
  ...
  - ブラウザやService Workerのキャッシュを避けるため、`https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.4.0` のようにクエリを付けて開く。
