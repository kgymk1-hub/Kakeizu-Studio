...
- **標準CSVセット**
  - Excelや外部ツールで編集しやすい、複数CSV + `manifest.json` の形式です。Kakeizu StudioからはZIPとしてエクスポートできます。
  - 人物・夫婦関係・親子関係に加えて、資料（Source）・出典（Citation）も含められます。
  - エクスポートファイル名は `kakeizu_standard_csv_set.zip` です。Kakeizu Studioが出力したZIPはそのまま再インポートできます。
  - Excel等で編集する場合はZIPを展開してCSVを編集し、再インポート時は再ZIP化せず `manifest.json` と各CSVを複数ファイル選択で直接インポートできます。
...
### ZIPインポートの互換性と複数ファイル直接インポート

現時点のZIP読込は無圧縮ZIP前提です。Kakeizu Studioが出力した標準CSVセットZIPは再インポート可能ですが、外部ツールで再圧縮したZIPは読めない場合があります。

外部ツールで再圧縮したZIPが読めない場合や、Excel等でCSVを編集した後は、ZIPに戻さず以下の6ファイルをまとめて複数ファイル選択で直接インポートしてください。
...
複数ファイル直接インポートでもZIPインポートと同じ検証・反映前プレビューを使用します。errorがある場合は反映せず、warningのみの場合は確認後に全置き換えで反映できます。
