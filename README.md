...
## v0.5 development: 家系図表示・出力仕上げ

v0.5 development では、既存のSVG家系図ビューをベースに、人物ノード、関係線、出力用見た目設定、PNG/PDF/SVG出力導線の見栄えを段階的に強化しています。新しいDBテーブルや本格レイアウトエンジンを追加する段階ではなく、package.json の version と README 冒頭はまだ `0.4.0` のままです。
...
- package.json の version はまだ変更していません。v0.5.0への変更は次フェーズ「v0.5.0リリース固定」で行います。
