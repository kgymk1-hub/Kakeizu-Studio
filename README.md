...
> v0.5.0は「家系図表示・出力の見栄え強化」リリースです。表示密度切替、人物ノードの生没年・出典・確度・確認状態表示、関係線の種類・状態反映、出力タイトル・凡例・背景設定、PNG/PDF/SVG出力を正式版として整理しました。

## 公開URL

- GitHub Pages: <https://kgymk1-hub.github.io/Kakeizu-Studio/>
- v0.5.0 キャッシュ回避確認: <https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.5.0>
...
## v0.6 development: 全体仕上げ確認 第7フェーズ

v0.6 development は、v0.6.0正式固定前に「検索・一覧・検証結果からの修正導線」をまとめて確認する開発中ステージです。第1〜第6フェーズで、共通の `target_type` / `target_id` 選択基盤、Person / Event / Source / Citation の一覧・検索、ValidationPanelから修正対象へ向かう導線、一覧系UIの見た目整理を段階的に追加しました。正式なVersion 0.6.0固定、package version更新、アプリヘッダー更新、リリースノート正式化は次フェーズで行います。

- 第1フェーズ: `SelectableTargetType` / `SelectableTarget`、`resolveSelectableTargetToPersonId`、`validationIssueToSelectableTarget`、App側 `selectTarget` を追加し、person / event / union / relation から人物詳細へ誘導する共通選択基盤を用意しました。source / citation は専用詳細画面がないため安全に扱います。
- 第2フェーズ: `PersonListPanel` を追加し、氏名・かな・旧姓・称号・職業・備考の検索、gender / confidence / review_status / 出典ありなしフィルタ、人物クリックによる人物詳細移動、件数表示・0件表示を整理しました。
- 第3フェーズ: `EventListPanel` を追加し、event_typeフィルタ、日付・説明・備考・関連人物名検索、関連人物名表示、Eventクリックによる対象移動、件数表示・0件表示を整理しました。
- 第4フェーズ: `SourceCitationPanel` を追加し、Source一覧、Citation一覧、source_typeフィルタ、Citation target_typeフィルタ、Source / Citation検索、Citation対象名表示、Citationクリックによる対応済み対象への移動、Sourceクリックによる関連Citation絞り込みを整理しました。
- 第5フェーズ: `ValidationPanel` に `onSelectTarget` を接続し、person / event / union / relation issueから対象人物へ誘導する「対象へ移動」ボタンを追加しました。source / citation issueは現時点では対象へ移動不可として表示します。
- 第6フェーズ: 共通list系CSS、検索欄・フィルタ・件数表示・0件表示の見た目、左側パネル縦長化、スマホ幅での最低限の折り返し、Source / Citation issue導線表示を仕上げました。
- v0.6 development中も、README冒頭、Appヘッダー、`package.json` versionは引き続き `0.5.0` のままです。
- JSON backup `schema_version`、Dexie schema version、DBテーブル、標準CSVセット構造は変更していません。
- 仕様は `docs/specification.md` を現行の参照先とします。古い詳細仕様書ファイル名ではなく、このファイルを基準に確認します。
...
- [ ] v0.5の家系図表示・PNG/PDF/SVG出力が維持されている。

### v0.6時点の未対応事項

- Event詳細編集UIの本格ジャンプは未対応です。
- Source詳細画面は未対応です。
- Citation詳細画面は未対応です。
...
- Source / Citation issueは専用詳細への直接ジャンプがまだないため、現時点では対象へ移動不可として安全に表示します。
...
- 共通の選択対象として Person / Event / Union / Relation / Source / Citation を扱う準備をします。
- Person target は既存の人物詳細パネルを開く人物選択へ解決します。
- Event target は、現時点では `target_type: "person"` のEventから関連Personへ誘導します。
- Union target は、関係者の人物詳細へ誘導できるようpartner人物へ解決します。
- Relation target は、親子関係の子または親の人物詳細へ誘導します。
- Source / Citation target は後続フェーズの専用一覧に接続しやすいよう、安全に受け付けます。
- ValidationPanelクリック接続は後続フェーズで本格対応するため、ValidationIssueから選択対象へ変換しやすい土台を用意します。
- このフェーズでは新しいDBテーブルは追加しません。
- JSON `schema_version` は変更しません。
- Dexie schema version は変更しません。
- `package.json` version は `0.5.0` のまま変更しません。
...
- キャッシュ回避確認: <https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.5.0>
...
- 公開ページが真っ白画面になる場合は、以下を確認してください。
  ...
  - ブラウザやService Workerのキャッシュを避けるため、`https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.5.0` のようにクエリを付けて開く。
