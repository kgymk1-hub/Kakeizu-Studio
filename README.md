...
> v0.3.0では、関係単位Citation UI、関係削除UI、関係編集UIに対応しました。人物詳細画面で親子関係・夫婦関係の根拠を管理し、関係の属性編集と安全な削除を行えます。

## 公開URL

- GitHub Pages: <https://kgymk1-hub.github.io/Kakeizu-Studio/>
- v0.3.0 キャッシュ回避確認: <https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.3.0>
...
### v0.4 development 第3フェーズ: 検証結果フィルタ・絞り込み最小版

v0.4 development の第3フェーズとして、検証ロジックを大きく増やさず、`validateFamilyData` の結果を確認する `ValidationPanel` に最小限の絞り込みUIを追加しました。package.json の version と JSON バックアップの schema_version はこの段階では変更しません。
...
- フィルタ適用後の「表示中」件数と全体件数を確認できます。
- 大量issueの場合は、フィルタ後の結果に対して最初の50件を表示します。
- issueクリックで対象へジャンプする機能はまだ未対応です。
- 自動修正は未対応です。

### v0.4 development 第4フェーズ: 日付・年齢チェック強化最小版

v0.4 development の第4フェーズとして、`validateFamilyData` の4桁西暦年ベースの日付・年齢チェックを少し強化しました。package.json の version と JSON バックアップの schema_version はこの段階では変更しません。
...
JSONバックアップはアプリ内部形式そのものです。v0.3.0では `schema_version` は必要がなければ `1.2` のままで、`persons` / `unions` / `parent_child_relations` / `import_batches` / `sources` / `citations` / `events` をまとめて保持します。旧 `1.0` / `1.1` JSONで `events` が存在しない場合は空配列として扱います。普段のバックアップや、Kakeizu Studio間で完全に復元したい場合に向いています。
...
- 公開ページが真っ白画面になる場合は、以下を確認してください。
  ...
  - ブラウザやService Workerのキャッシュを避けるため、`https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.3.0` のようにクエリを付けて開く。
...
- キャッシュ回避確認: <https://kgymk1-hub.github.io/Kakeizu-Studio/?v=0.3.0>
...
v0.3.0として区切る場合は、mainブランチ上で以下を実行してください。
...
GitHub上でReleaseを作る場合は、tag `v0.3.0` を使ってReleaseを作成してください。Release本文の下書きは `RELEASE_NOTES.md` の「GitHub Release body」を利用できます。
