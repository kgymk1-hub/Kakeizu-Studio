...
Version 1.4 では、Version 1.3 の基本設計を維持しつつ、Kakeizu Studio v0.8.0 までの実装状況を反映する。

v0.8.0 時点では、以下が実装済みまたは最小版実装済みである。
...
- 表示設定・出力設定のDB永続化
- 公開用出力モード最小版
- JSON backup schema_version 1.3
...
↓
将来的にName / Place、GEDCOM、親等計算、AI/OCRへ拡張
```
...
| CSV処理 | PapaParse | 採用 |
| バリデーション | Zod / 独自検証 | 採用 |
| テスト | Vitest | 採用。v0.8.0時点では安定実行のため `vitest run --pool=threads --fileParallelism=false` を使用。 |
| 家系図描画 | SVG | 採用 |
...
v0.8.0までで、基本的な家系データ管理、家系図表示・出力、検索・一覧・修正導線、CSVインポートの取込前プレビュー・検証・履歴・結果レポート、Project/settingsの最小永続化、公開用出力モード最小版まで到達した。

今後は、Name / Place の最小版、安定版仕上げへ進む。

| バージョン | 内容 |
|---|---|
| v0.9.0 | Name / Place 最小版 |
| v1.0.0 | 戸籍・出典管理対応の安定版 |
| v1.1以降 | GEDCOM、親等計算、相続関係説明図、OCR/AI支援、複数Project本格対応、複数ビュー本格対応 |

### 9.1 v0.9.0 の目標：Name / Place 最小版
...
- Nameモデル
- Placeモデル
- Dexie schema version(5) の追加検討
- JSON backup schema_version 1.4 の追加検討
- PersonとのName連携
- EventとのPlace連携
- Source / Citationとの連携準備
- Name / Placeの検索・一覧最小版
- Citation target_typeとしての name / place の本格化準備
- CSV / JSON互換性整理

### 9.2 v1.0.0 の目標：安定版仕上げ
...
v0.8.0時点では、Projectは設定管理の器であり、人物・関係・出来事・出典のデータ分離単位ではない。
...
v0.8.0時点では、Personに `project_id` は付与しない。将来の複数Project本格対応時に検討する。
...
v0.8.0時点では、Eventは `target_type` と `target_id` を持ち、person / union / relation を対象にできる設計で実装されている。Event一覧から対象へ移動する最小導線も実装済みである。
...
v0.8.0時点では `name` / `place` は将来対象として型・設計上の余地を残す。実データとしてのName / Place本格管理はv0.9.0以降で扱う。
...
別名、旧名、戒名、幼名、通称、諱などを管理する。v0.9.0で最小版を予定する。

### 23.2 Place

本籍地、出生地、死亡地、居住地、埋葬地などを管理する。v0.9.0で最小版を予定する。
...
v0.8.0時点では、基本的なCSVインポート、標準CSVセット入出力、複数CSV直接インポート、取込前プレビュー、取込方式選択、external_id照合、参照先不明検出、標準CSVセット検証、ImportBatch、ImportReportを実装済みである。

v0.8.0では標準CSVセット構造は変更していない。
...
ただし、v0.8.0時点では標準CSVセット構造は変更していない。

---

## 28. 取込方式

v0.8.0時点で、取込方式選択UIを実装済みである。
...
v0.8.0時点で、実際に取込実行できる条件は以下のみである。
...
v0.8.0時点で、参照先不明・仮人物作成方針UIを実装済みである。
...
v0.8.0時点では、JSON backup `schema_version` は `1.3` である。Project / ViewSetting / ExportSetting / PrivacySetting を含めるため、v0.8.0で `1.2` から `1.3` に更新した。
...
v0.8.0では、1.0 / 1.1 / 1.2 / 1.3 の復元互換を維持する。古いJSONにProject / settingsが存在しない場合は、default project/settingsを補完する。

### 30.2 v0.8.0時点で含める主な内容
...
v0.8.0時点で、Project / ViewSetting / ExportSetting / PrivacySetting の最小UIを提供する。

### 33.1 Project表示

- 現在のProject名を表示する。
- v0.8.0時点では単一default project相当であり、複数Project切替UIは未対応。
...
v0.8.0時点では、簡易独自レイアウトを採用する。ELK.js / React Flow / 複数ビュー本格対応は将来対応とする。

---

## 37. 対応ビュー

v0.8.0時点では、基本の家系図ビューを実装済みである。
...
v0.8.0時点で、人物ノード表示密度を切り替えられる。表示密度はViewSettingとしてDB保存・復元される。
...
v0.8.0時点で、親子関係・Union関係の種別や状態を線で表現する。
...
v0.8.0時点では、ViewSetting.show_relation_legend と ExportSetting.show_legend の責務は分けているが、現行DOMキャプチャ方式のため画面凡例と出力凡例は連動扱いである。
...
v0.8.0時点で、出力用見た目設定はExportSettingとしてDB保存・復元される。
...
v0.8.0時点で、表示中の家系図DOMをPNGとして出力する。
...
v0.8.0時点で、html2canvas + jsPDF によりPDF出力する。
...
v0.8.0時点で、検証エンジンとValidationPanelを実装済みである。
...
v0.8.0時点では、PrivacySettingと公開用出力モード最小版を実装済みである。
...
v0.8.0時点では、検証用途として4桁西暦抽出の最小対応を実装済みである。
...
v0.8.0時点で、Vitestにより主要機能のテストが整備されている。
...
39. JSON backup schema_version 1.3でProject / settingsを保存できる。
40. JSON 1.2以前の復元互換を維持する。
41. Dexie schema version(4)でProject / settingsテーブルを持つ。
42. 標準CSVセット構造を変更していない。
43. Vitestで主要機能のテストが通る。
...
v0.8.0までの開発では、この分割方針が有効だった。今後も同じ方針を維持する。

---

## 62. 今後の推奨プロンプト順

v0.8.0までの開発では、小さなフェーズに分割して、各段階でテストとビルドを通す進め方が有効だった。今後も同じ方針を維持する。

### v0.9.0：Name / Place 最小版

- v0.9 第0フェーズ：仕様書v1.4をdocsへ反映
- v0.9 第1フェーズ：Name / Place現状棚卸し
- v0.9 第2フェーズ：Nameモデル最小版
- v0.9 第3フェーズ：PersonとNameの最小連携
- v0.9 第4フェーズ：Placeモデル最小版
- v0.9 第5フェーズ：Event / Source とPlaceの最小連携
- v0.9 第6フェーズ：Name / Place一覧・検索最小版
- v0.9 第7フェーズ：Citation target_type name / place の準備
- v0.9 第8フェーズ：JSONバックアップ・DB互換性確認
- v0.9 第9フェーズ：v0.9全体仕上げ確認
- v0.9 第10フェーズ：v0.9.0リリース固定
...
↓
将来的にName / Place・GEDCOM・親等計算・相続関係説明図へ拡張できる
```
...
10. Name / Placeはv0.9.0で最小版として段階導入する。
...
v0.8.0時点で、CSVから家系図を作るMVP、戸籍・出典・Event管理、関係根拠、検証機能、家系図表示・出力強化、検索・一覧・検証結果からの修正導線、CSVインポートの取込前確認・履歴・結果レポート、Project/settingsの最小永続化、公開用出力モード最小版まで到達している。

今後は、Name / Place最小版と安定版仕上げへ進むことで、単なる家系図作図アプリではなく、戸籍調査と出典管理に耐える家系データ管理アプリへ育てていく。
