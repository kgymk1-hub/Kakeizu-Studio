...
| Version 1.6 | Kakeizu Studio v1.0.0リリース候補の安定版仕上げを反映。v1到達点棚卸し、既知制限、フル機能サンプル、JSON schema 1.0〜1.4互換fixture・テスト、手動確認チェックリスト、テスト範囲整理、旧schema再出力時のschema 1.4固定、GitHub Pages・リリース前確認方針を反映。 |
...
Version 1.6 では、Version 1.5.1までの実装反映内容を維持しつつ、v1.0.0 Release Candidateとしての安定版仕上げ、文書整備、サンプル・fixture・テスト棚卸し、GitHub Pages技術確認、リリース前確認方針を整理する。
...
## 0.3 JSONバックアップ仕様の現在状態

- 現行出力: schema 1.4
- 復元対応: schema 1.0〜1.4
- 旧schema復元時: 不足配列・settings補完
- 再出力時: schema 1.4へ更新
- unsupported schema: 拒否
...
### 8.10 v1.0.0リリース候補：安定版仕上げ

v1.0.0 Release Candidateでは、production codeの大型機能追加ではなく、v0.1.0〜v0.9.0到達点を安定版候補として整理し、文書・サンプル・互換性・テスト・公開確認の整合を確認した。

#### 8.10.1 v1.0準備 第0〜0-5
...
- サンプルImportBatchのmetadata整合修正
- `total_rows`と`imported_counts`の一致
- 標準CSVセット由来Citationを23件として整理

#### 8.10.4 統合B事前補完

- 旧schema復元後の再出力をschema 1.4へ固定
- 再出力時の新しい`exported_at`
- 回帰テスト追加

#### 8.10.5 統合B

- schema 1.0〜1.4代表fixture
- 互換性テスト19件
- 手動チェックリスト50項目
- テスト範囲棚卸し
...
現在はv1.0.0 Release Candidateである。収録範囲確定、到達点棚卸し、既知制限、サンプル、schema互換性、自動テスト、文書整理、GitHub Pages技術確認は完了している。実ブラウザ手動確認、手動チェックリスト記録、package / App / READMEの1.0.0固定、tag、GitHub Releaseは未完了であり、統合Dで扱う。
...
完全復元用形式としてJSONバックアップを実装する。現行出力は`schema_version: "1.4"`である。
...
復元対応はschema 1.0〜1.4である。旧schema復元時は、不足配列とProject / settingsを補完する。旧schemaを復元後に再出力する場合、再出力JSONは必ずschema 1.4になり、`exported_at`は再出力時の新しい日時になる。unsupported schemaは拒否する。

### 30.2 schema別の代表確認内容

| schema | 代表fixtureで確認する内容 |
|---|---|
| 1.0 | Person中心 |
| 1.1 | Source / Citation |
| 1.2 | Event |
...
互換性fixtureは各schemaの代表的な最小構成である。過去の全出力形式や全フィールド構成を完全に再現・保証するものではない。参照資料は`docs/backup_compatibility_v1.0.md`と`samples/compatibility/`である。

### 30.3 v0.9.0時点で含める主な内容
...
### 58.9 v1.0 Release Candidateの確認基準
...
- 手動チェックリスト50項目を別枠で管理する。
- GitHub Pages技術確認を実施済みとして扱う。
- 実ブラウザ確認は自動テストとは別枠であり、Release Candidate時点では未実施である。
...
### 60.1 v1.0.0 Release Candidateの受け入れ分類
...
- TypeScript成功
- v1フル機能サンプル成功
- schema 1.0〜1.4代表fixture成功
- DB / JSON / CSV schema固定
- conflict markerなし
- GitHub PagesのHTML・asset・base確認
...
- P0にBlockerなし
- P1の実施結果を記録
- package / App / README versionを1.0.0へ固定
- tag作成
- GitHub Release作成
