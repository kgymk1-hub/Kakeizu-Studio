...
Version 1.1 では、Version 1.0 の基本設計を維持しつつ、Kakeizu Studio v0.5.0 までの実装状況を反映する。

v0.5.0 時点では、以下が実装済みまたは最小版実装済みである。
...
| Person | 実装済み。人物基本情報を管理。 |
| Name | 将来対応。 |
| Union | 実装済み。婚姻・パートナー関係・不明な親組み合わせを管理。 |
| ParentChildRelation | 実装済み。親子関係を人物から分離して管理。 |
| Event | 実装済み。出生・死亡・婚姻・離婚・養子縁組・転籍等を管理。 |
| Source | 実装済み。戸籍・Web・書籍・AI生成データ等の資料を管理。 |
| Citation | 実装済み。人物・Event・Union・Relationへの根拠付けを管理。 |
| Media | 将来対応。 |
| Place | 将来対応。 |
| ImportBatch | 一部設計済み。将来本格対応。 |
...
- Citation一覧
- Relation / Union一覧
- 検証結果クリックから対象人物・関係・Eventへジャンプ
- 出典なし、未確認、低確度、日付矛盾などの修正導線
- 一覧から詳細パネルを開く導線
- 検索・フィルタ・ソートの最小版
...
v0.5.0時点では、Eventは `target_type` と `target_id` を持ち、person / union / relation を対象にできる設計で実装されている。
...
v0.5.0時点では本格永続化していない。将来、プロジェクト単位の設定、表示設定、出力設定、プライバシー設定をDBに保存する。
...
v0.5.0時点では、基本的なCSVインポート、標準CSVセット入出力、複数CSV直接インポートが実装済みである。v0.7.0でCSVインポート本格化を予定する。
...
v0.5.0時点では、JSON backup `schema_version` は `1.2` のままである。v0.5.0の表示・出力強化はDBスキーマやバックアップスキーマの破壊的変更を伴わない。
...
v0.5.0時点では、簡易独自レイアウトを採用する。ELK.js / React Flow / 複数ビュー本格対応は将来対応とする。

---

## 27. 対応ビュー

v0.5.0時点では、基本の家系図ビューを実装済みである。
...
v0.5.0時点で、親子関係・Union関係の種別や状態を線で表現する。
...
v0.5.0時点で、出力用見た目設定を画面状態として管理する。
...
v0.5.0時点で、表示中の家系図DOMをPNGとして出力する。
...
v0.5.0時点で、html2canvas + jsPDF によりPDF出力する。
...
v0.5.0時点で、検証エンジンとValidationPanelを実装済みである。
...
v0.6.0では、検証結果から対象人物・関係・Eventへジャンプして修正できる導線を追加する。

---

## 36. 検索・一覧機能

v0.5.0時点では本格未対応。v0.6.0で最小版を実装予定。
...
v0.5.0時点では、検証用途として4桁西暦抽出の最小対応を実装済みである。
...
v0.5.0時点で、Vitestにより主要機能のテストが整備されている。
...
v0.5.0までの開発では、この分割方針が有効だった。今後も同じ方針を維持する。
...
↓
将来的に検索・一覧・GEDCOM・親等計算・相続関係説明図へ拡張できる
```
...
v0.5.0時点では、ユーザーが以下を実行できる状態を目指す。
...
7. 家系図をSVGで表示する。
8. 表示密度や関係線凡例で見やすく確認する。
9. 出力タイトル・凡例・背景を調整する。
10. 家系図をPNG/PDF/SVGで出力する。
11. データをCSV/JSONで保存する。
...
v0.5.0時点で、CSVから家系図を作るMVP、戸籍・出典・Event管理、関係根拠、検証機能、家系図表示・出力強化まで到達している。

今後は、検索・一覧・修正導線、CSVインポート本格化、Project / ViewSetting / ExportSetting、Name / Placeへ進むことで、単なる家系図作図アプリではなく、戸籍調査と出典管理に耐える家系データ管理アプリへ育てていく。
